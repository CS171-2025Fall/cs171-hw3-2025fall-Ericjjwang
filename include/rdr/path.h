...
public:
   ...
   * Is not practical to perform the throughput and pdf calculation separately
   * since their value can be very small with geometric terms.
   *
   * @return Float
   */
  ...
  /**
   * @brief Translate between different measures. Given this interaction and the
...
protected:
  ...
  PathInterface &operator=(const PathInterface &other) {
...
// The cached version is used for debug only now
class PathImmediate final : PathInterface<PathImmediate> {
public:
  using Super = PathInterface<PathImmediate>;
  using Super::toPdfMeasure;

  /// Construct with the first ray.
  PathImmediate(const Ray &ray, const PathIntegrator *integrator)
      : Super(ray, integrator) {}

  /// @see PathInterface::addInteraction
  PathImmediate &addInteraction(
      const SurfaceInteraction &next_interaction) override;

  /// @see PathInterface::estimate
  Vec3f estimate() const override {
    return cached_result * mis_weight * rr_weight;
  }
  ...
  /// @see PathInterface::length
  int length() const override { return cached_length; }
  ...
  /// @see PathInterface::toString
  std::string toString() const override { return format("PathImmediate[]"); }

private:
  bool is_terminated{false};
  int cached_length{0};
  Vec3f cached_throughput{1.0}, cached_result{0.0};
  Float mis_weight{1.0}, rr_weight{1.0};
